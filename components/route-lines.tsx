...
export default function RouteLines({linePoints1, linePoints2}: {linePoints1: Vector3[], linePoints2: Vector3[]}) {
    ...
    useFrame(() => {
        ...
        for(let i = 0; i < linePoints1.length; i++) {
            positionAttribute.setXYZ(i, linePoints1[i].x, linePoints1[i].y, linePoints1[i].z);
        }
        positionAttribute.needsUpdate = true;

        const positionAttribute2 = route2Ref.current.getAttribute("position");
        for(let i = 0; i < linePoints2.length; i++) {
            positionAttribute2.setXYZ(i, linePoints2[i].x, linePoints2[i].y, linePoints2[i].z);
        }
